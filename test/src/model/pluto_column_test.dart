...
void main() {
  group('titleWithGroup', () {
    ...
    test('group 이 여러개 있는 경우 컬럼 제목과 그룹 명이 모두 리턴 되어야 한다.', () {
      ...
      final group = PlutoColumnGroup(title: 'groupA2-1', fields: ['column']);

      PlutoColumnGroup(
        title: 'groupA',
        children: [
          PlutoColumnGroup(title: 'groupA1', fields: ['DUMMY_COLUMN']),
          PlutoColumnGroup(title: 'groupA2', children: [group]),
        ],
      );
  ...
  group('PlutoColumnTypeText 의 defaultValue', () {
    test('text 기본 값이 설정 되어야 한다.', () {
      final PlutoColumnTypeText column =
          PlutoColumnType.text(defaultValue: 'default') as PlutoColumnTypeText;

      expect(column.defaultValue, 'default');
    });

    test('number 기본 값이 설정 되어야 한다.', () {
      final PlutoColumnTypeNumber column =
          PlutoColumnType.number(defaultValue: 123) as PlutoColumnTypeNumber;

      expect(column.defaultValue, 123);
    });

    test('select 기본 값이 설정 되어야 한다.', () {
      final PlutoColumnTypeSelect column =
          PlutoColumnType.select(<String>['One'], defaultValue: 'One')
              as PlutoColumnTypeSelect;

      expect(column.defaultValue, 'One');
    });

    test('date 기본 값이 설정 되어야 한다.', () {
      final PlutoColumnTypeDate column =
          PlutoColumnType.date(defaultValue: DateTime.parse('2020-01-01'))
              as PlutoColumnTypeDate;

      expect(column.defaultValue, DateTime.parse('2020-01-01'));
    });

    test('time 기본 값이 설정 되어야 한다.', () {
      final PlutoColumnTypeTime column =
          PlutoColumnType.time(defaultValue: '20:30') as PlutoColumnTypeTime;

      expect(column.defaultValue, '20:30');
    });
  });

  group('PlutoColumnTypeText', () {
    test('text', () {
  ...
  group('PlutoColumnTypeNumber', () {
    ...
    group('isValid', () {
      ...
      test('negative 가 false 이고 -123 인 경우 false', () {
        final PlutoColumnTypeNumber numberColumn =
            PlutoColumnType.number(negative: false) as PlutoColumnTypeNumber;
  ...
  group('PlutoColumnTypeSelect', () {
    group('isValid', () {
      test('items 에 포함 되어있으면 true', () {
        final PlutoColumnTypeSelect selectColumn =
            PlutoColumnType.select(<String>['A', 'B', 'C'])
                as PlutoColumnTypeSelect;
        expect(selectColumn.isValid('A'), isTrue);
      });

      test('items 에 포함 되어 있지 않으면 false', () {
        final PlutoColumnTypeSelect selectColumn =
            PlutoColumnType.select(<String>['A', 'B', 'C'])
                as PlutoColumnTypeSelect;
    ...
    group('compare', () {
      test('Two, Three 인 경우 -1', () {
        final PlutoColumnTypeSelect column =
            PlutoColumnType.select(<String>['One', 'Two', 'Three'])
                as PlutoColumnTypeSelect;
        expect(column.compare('Two', 'Three'), -1);
      });

      test('Three, Two  인 경우 1', () {
        final PlutoColumnTypeSelect column =
            PlutoColumnType.select(<String>['One', 'Two', 'Three'])
                as PlutoColumnTypeSelect;
  ...
  group('PlutoColumnTypeDate', () {
    group('isValid', () {
      ...
      test('시작일이 있는 경우 시작일 보다 작으면 false', () {
        final PlutoColumnTypeDate dateColumn =
            PlutoColumnType.date(startDate: DateTime.parse('2020-02-01'))
                as PlutoColumnTypeDate;
        expect(dateColumn.isValid('2020-01-01'), isFalse);
      });

      test('시작일이 있는 경우 시작일과 같으면 true', () {
        final PlutoColumnTypeDate dateColumn =
            PlutoColumnType.date(startDate: DateTime.parse('2020-02-01'))
                as PlutoColumnTypeDate;
      ...
      test('마지막일이 있는 경우 마지막일보다 작으면 true', () {
        final PlutoColumnTypeDate dateColumn =
            PlutoColumnType.date(endDate: DateTime.parse('2020-02-01'))
                as PlutoColumnTypeDate;
        expect(dateColumn.isValid('2020-01-01'), isTrue);
      });

      test('마지막일이 있는 경우 마지막일과 같으면 true', () {
        final PlutoColumnTypeDate dateColumn =
            PlutoColumnType.date(endDate: DateTime.parse('2020-02-01'))
                as PlutoColumnTypeDate;
        expect(dateColumn.isValid('2020-02-01'), isTrue);
      ...
      test('마지막일이 있는 경우 마지막일보다 크면 false', () {
        final PlutoColumnTypeDate dateColumn =
            PlutoColumnType.date(endDate: DateTime.parse('2020-02-01'))
                as PlutoColumnTypeDate;
      ...
      test('시작일과 마지막일이 둘다 있는 경우 범위에 있으면 true', () {
        final PlutoColumnTypeDate dateColumn =
      ...
      test('시작일과 마지막일이 둘다 있는 경우 범위보다 작으면 false', () {
        final PlutoColumnTypeDate dateColumn =
            PlutoColumnType.date(
                  startDate: DateTime.parse('2020-02-01'),
                  endDate: DateTime.parse('2020-02-05'),
      ...
      test('시작일과 마지막일이 둘다 있는 경우 범위보다 크면 false', () {
        final PlutoColumnTypeDate dateColumn =
            PlutoColumnType.date(
                  startDate: DateTime.parse('2020-02-01'),
                  endDate: DateTime.parse('2020-02-05'),
    ...
    group('compare', () {
      test('2019-12-30, 2020-01-01 인 경우 -1', () {
        final PlutoColumnTypeDate column =
            PlutoColumnType.date() as PlutoColumnTypeDate;
      ...
      test('12/30/2019, 01/01/2020 인 경우 1', () {
        final PlutoColumnTypeDate column =
            PlutoColumnType.date(format: 'MM/dd/yyyy') as PlutoColumnTypeDate;
        expect(column.compare('12/30/2019', '01/01/2020'), 1);
      });

      test('2020-01-01, 2019-12-30  인 경우 1', () {
        final PlutoColumnTypeDate column =
            PlutoColumnType.date() as PlutoColumnTypeDate;
      ...
      test('2020-01-01, 2020-01-01 인 경우 0', () {
        final PlutoColumnTypeDate column =
            PlutoColumnType.date() as PlutoColumnTypeDate;
        expect(column.compare('2020-01-01', '2020-01-01'), 0);
      });

      test('01/01/2020, 01/01/2020  인 경우 0', () {
        final PlutoColumnTypeDate column =
            PlutoColumnType.date(format: 'MM/dd/yyyy') as PlutoColumnTypeDate;
  ...
  group('formattedValueForType', () {
    test('Number column 인 경우 12345 값이 12,345 로 포멧이 적용 되어야 한다.', () {
    ...
    test('Number column 인 경우 12345678 값이 12,345,678 로 포멧이 적용 되어야 한다.', () {
      final PlutoColumn column = PlutoColumn(
        title: 'number column',
        field: 'number_column',
        type: PlutoColumnType.number(), // 기본 포멧 (#,###)
    ...
    test('Number column 이 아닌 경우 12345 값이 12345 로 포멧이 적용 되지 않아야 한다.', () {
      final PlutoColumn column = PlutoColumn(
        title: 'number column',
        field: 'number_column',
  ...
  group('formattedValueForDisplay', () {
    test('formatter 가 없는 경우 Number column 의 타입에 대한 포멧만 적용되어야 한다.', () {
      final PlutoColumn column = PlutoColumn(
        title: 'number column',
        field: 'number_column',
        type: PlutoColumnType.number(), // 기본 포멧 (#,###)
    ...
    test('formatter 가 있는 경우 Number column 의 타입에 대한 포멧이 아닌 '
  ...
  group('formattedValueForDisplayInEditing', () {
    group('PlutoColumnTypeWithNumberFormat 이 아닌 컬럼.', () {
      ...
      test('readOnly 가 false 여도 formatter 가 있고 select 타입 컬럼인 경우 '
          'formatter 가 적용 되어야 한다.', () {
        final column = PlutoColumn(
          title: 'column',
          field: 'column',
      ...
      test('readOnly 가 false 여도 formatter 가 있고 time 타입 컬럼인 경우 '
          'formatter 가 적용 되어야 한다.', () {
        final column = PlutoColumn(
          title: 'column',
          field: 'column',
      ...
      test('readOnly 가 false 여도 formatter 가 있고 date 타입 컬럼인 경우 '
          'formatter 가 적용 되어야 한다.', () {
        final column = PlutoColumn(
          title: 'column',
          field: 'column',
      ...
      test('formatter 가 있고 readOnly 컬럼인 경우 applyFormatterInEditing 가 false 면 '
